...
#### 3. COCO bowl 클래스의 학습 데이터 편향

COCO 데이터셋의 bowl 이미지는 대부분 측면 또는 비스듬한 각도로 촬영되어 있습니다. 정면(위에서 내려다본) 촬영에서 인식률이 낮아 인식되지 않는 상황이 발생하기도 하는 것을 확인했습니다. 

- **보완 방향**

  > 정면 촬영 이미지를 추가 수집하여 파인튜닝하거나, bowl-circle 전용 데이터셋을 별도 구축하면 성능을 높일 수 있을 것이라 예상됩니다. 
