...
- **베이스 모델**: YOLOv8n-seg (COCO pretrained)
- **데이터셋**: Train 646장 / Validation 48장
- **설정**: 최대 70 epoch, patience 20 → 59 epoch에서 조기 종료

</br>

### 최종 성능 (epoch 59 기준, segmentation)
...
| 손실 | 초기 (epoch 1) | 최종 (epoch 59) |
|----------|----------------|-----------------|
| box_loss | 0.849 | 0.379 |
| seg_loss | 1.775 | 0.600 |
| cls_loss | 1.642 | 0.381 |

train/val loss 모두 꾸준히 감소하여 과적합 징후 없이 안정적으로 수렴했습니다.
...
사용환경마다 조도와 촬영 거리가 다를 것으로 예상되어 고정된 px/cm 값을 쓸 수 없었고, 신용카드가 국제규격이 정해져서 기준 물체로 활용될 수 있다는 걸 알게 되었습니다.
