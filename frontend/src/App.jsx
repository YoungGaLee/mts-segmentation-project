...
function LimitationNote() {
  return (
    <div className="limitation-note">
      <h4>💡 한계 및 개선 방향</h4>
      <p><strong>한계 1. 측면 촬영 시, 입구 지름 측정 불가 (rim 미학습)</strong></p>
      <p>
        현재 파이프라인에서는 객체 전체 마스크를 기반으로 지름을 산출하고 있어 측정값에 오차가 발생하고 있습니다.
        냄비(또는 그릇)의 원형 입구(rim)를 별도로 추출하여 학습시키지 못했기 때문입니다.
        (실제 11cm인 그릇이 약 12.5cm로 출력됨. 케이스에 따라 더 큰 오차 발생 예상)
      </p>
      <p>
        이를 개선하기 위해서는 입구 테두리(rim)를 별도의 클래스로 정의하고, 해당 영역에 대한 polygon 어노테이션을 추가하여 모델을 파인튜닝하면 지름 정확도를 크게 향상시킬 수 있을 것이라 예상합니다.
        Roboflow 등 클라우드 라벨링 서비스에서 rim polygon 라벨링은 자동화를 지원하지 않아
      <strong>1000여장이 넘는 데이터에 대해 수작업 필요했고, 시간 제약으로 인해 아쉽게 적용하지 못했습니다.</strong>
      </p>
      <p><strong>한계 2. 높이 차이로 인한 원근 오차</strong></p>
      <p>
        카드와 냄비를 바닥에 두고 위에서 촬영할 경우, 냄비 높이만큼 카메라와의 거리가 달라져
        원근법에 의해 냄비 입구가 카드보다 더 크게 보입니다. 이로 인해 냄비가 실제 크기보다 크게 측정되는 문제가 있습니다.
        <strong> 단, 카드를 냄비 입구와 같은 높이에 맞춰 측정하면 정확하게 측정됩니다.</strong>
      </p>
    </div>
  )
}
...
function ResultPanel({ result }) {
  ...
  return (
    <div className="result-panel">
      ...
      <div className="warning">위에서 촬영하면 더 정확하게 측정가능합니다.</div>
